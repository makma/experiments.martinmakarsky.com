 ...
 *
 * Note: In Next 16 route handlers, `context.params` is a Promise.
 */
export async function GET(
  request: NextRequest,
  context: { params: Promise<{ randomPathSegments: string[] }> }
) {
  try {
    const { randomPathSegments } = await context.params;
    ...
    const randomPath = randomPathSegments.join("/");
