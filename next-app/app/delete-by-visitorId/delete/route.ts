...
export async function DELETE(request: Request) {
  if (request.method !== "DELETE") {
    return Response.json({ message: `${request.method} is not a supported method.` }, { status: 400 });
  }

  const SEALED_RESULTS_DECRYPTION_KEY = process.env.SEALED_RESULTS_DECRYPTION_KEY;
  if (!SEALED_RESULTS_DECRYPTION_KEY) {
    return Response.json({ message: `Missing SEALED_RESULTS_DECRYPTION_KEY environment variable` }, { status: 500 });
  }
  ...
  if (!unsealedResult || !unsealedResult.products.identification || !unsealedResult.products.identification.data) {
    return Response.json({ message: `Invalid unsealed data.` }, { status: 400 });
  }
  ...
  if (requestVisitorId !== "phKwbdoV8NEAGTATwAiT") {
    return Response.json({ message: `${requestVisitorId} is not allowed to delete data` }, { status: 403 });
  }

  const visitorIdToDelete = body.visitorIdToDelete;

  if (!visitorIdToDelete) {
    return Response.json({ message: `${visitorIdToDelete} is not a visitorId.` }, { status: 400 });
  }

  if (visitorIdToDelete === unsealedResult.products.identification.data.visitorId) {
    return Response.json({ message: `Cannot delete since the visitorId to delete is the same as the current visitorId.` }, { status: 400 });
  }

  const secretAPIKey = process.env.FINGERPRINT_SEALED_RESULTS_SECRET_API_KEY
  if (!secretAPIKey) {
    return Response.json({ message: `Missing secret API key.` }, { status: 500 });
  ...
  if (response.status !== 200) {
    return Response.json({ message: { status: response.status, body: response.body } }, { status: 500 });
  }
  return Response.json({ message: `The visitorId: ${visitorIdToDelete} is scheduled to be deleted.` }, { status: 200 });
}
